...
const index = async function(req, res){
    ...
    try {
        ...
        // comprobamos que el usuario este asociado a la campaña
        const campanaQuery = "select c.nombre from campana as c join equipo as e on e.idcampana=c.idcampana where c.idcampana=$1 and e.idusuario=$2"
        let {rows:[campana]} = await db.query(campanaQuery, [idCampaign, idusuario]);
        if (campana == undefined) {
            throw "no asociados";
        }

        const productsQuery = "select * from producto where idcampana=$1"
        let {rows:productos = []} = await db.query(productsQuery, [idCampaign]);

        // res.render('order/create', {data:response.rows, query:{id1:idCampaign, id2:idusuario}});
        res.render('order/create', {campana, productos, query:{id1:idCampaign, id2:idusuario}});
